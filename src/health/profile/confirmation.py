...
Section application coverage in this phase:

  add → list sections (conditions/medications/allergies/family_history/
        labs/concerns)
  modify (set field) → demographics + lifestyle

  remove + modify-existing-list-item land in a follow-up — they need
  more interactive design (which entry? full vs partial replacement?)
  that doesn't add value over re-typing for the MVP.
"""
...
def _extracted_provenance(value: dict, now: datetime) -> ProfileField:
    """ProfileField wrapper for an accepted extractor proposal."""
