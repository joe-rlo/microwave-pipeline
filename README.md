...
- **`medical_literature_search`** / **`medical_article_fetch`** — biomedical literature retrieval for health questions. Search hits **Europe PMC** by default (PubMed/MEDLINE + PMC full text + preprints in one index; `MEDICAL_SEARCH_BACKEND=pubmed` switches to NCBI E-utilities). Fetch returns the abstract always, **open-access full text** via NCBI's BioC service when the article is in the OA subset, and a **legally-posted free PDF/link** resolved through **Unpaywall** when the publisher copy is paywalled. Always-on (no API key); set `RESEARCH_CONTACT_EMAIL` so Unpaywall/NCBI can identify the caller, or `MEDICAL_TOOLS_DISABLED=true` to remove them. **Every source is sanctioned** — Europe PMC and NCBI are public-funded, Unpaywall only indexes versions the authors/publishers posted legally. No paywall-bypassing or pirate mirrors (e.g. Sci-Hub): when nothing open or free exists, the tool returns the abstract and the publisher link and says so, rather than reaching around the paywall.
